...
stab_explainer.gstab_lm <- function(object) {
  ...
  stability_definitions <- list(
    ...
    "Stability under Data Selection" = list(
      ...
      2. Data with outliers removed, created by using robust multivariate Mahalanobis distances. If outlier identification using robust multivariate Mahalanobis distances fails, then an observation will be identified as an outlier if it falls outside Q1 - 1.5*IQR or above Q3 + 1.5*IQR for any of the variables in the data, where Q1 and Q3 represent the first and third quartiles, and IQR is the interquartile range. Outliers are then removed from the data and the model is refitted.
      ...
      Secondly, the function uses robust multivariate Mahalanobis distances to identify outliers, with a fallback to a simpler method based on the interquartile range. Both methods have limitations and may not identify all outliers, particularly in complex or high-dimensional data. In addition, the removal of outliers is not always the best strategy, especially if those outliers are important or informative for the analysis.
